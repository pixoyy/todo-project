...
@section('main-content')
    <x-custom-card>
        <form id="form-task" class="row g-3" method="POST" action="{{ route('tasks_update', $task->id) }}">
            @csrf
            @method('PATCH')
                ...
                <select name="project_id" id="project_id"
                    ...
                    @foreach ($projects as $project)
                        <option value="{{ $project->id }}"
                            @selected(old('project_id', $task->category->project_id ?? '') == $project->id)>{{ $project->name }}</option>
                    @endforeach
                </select>
                @error('project_id')
            ...
            <div class="col-12">
                ...
                <input type="text" name="title" id="title"
                    class="form-control border-grey @error('title') is-invalid @enderror"
            ...
            <div class="col-12">
                ...
                <textarea name="description" id="description" cols="30" rows="8"
                    class="form-control border-grey @error('description') is-invalid @enderror">{{ old('description', $task->description) }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="col-12 col-md-3">
                ...
                <select name="priority" id="priority"
                    class="form-select border-grey @error('priority') is-invalid @enderror" required>
            ...
            <div class="col-12 col-md-3">
                ...
                <input type="date" name="due_date" id="due_date"
                    class="form-control border-grey @error('due_date') is-invalid @enderror"
                ...
                <select name="assigned_admin_id" id="assigned_admin_id"
                    ...
                    @foreach ($admins as $admin)
                        <option value="{{ $admin->id }}"
                            @selected(old('assigned_admin_id', $task->assigned_admin_id) == $admin->id)>{{ $admin->name }}</option>
                    @endforeach
                </select>
...
@push('js')
    ...
    <script>
        ...
        const oldProjectId = "{{ old('project_id', $task->category->project_id ?? '') }}";
        const oldCategoryId = "{{ old('category_id', $task->category_id) }}";
