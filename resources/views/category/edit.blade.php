...
@section('main-content')
    <x-custom-card>
        <form class="row g-3" method="POST" action="{{ route('categories_update', $category->id) }}">
            @csrf
            @method('PATCH')
                ...
                <select name="project_id" id="project_id"
                    ...
                    @foreach ($projects as $project)
                        <option value="{{ $project->id }}"
                            @selected(old('project_id', $category->project_id) == $project->id)>{{ $project->name }}</option>
                    @endforeach
                </select>
                @error('project_id')
            ...
            <div class="col-12 col-md-6">
                ...
                <input type="text" name="name" id="name"
                    class="form-control border-grey @error('name') is-invalid @enderror"
            ...
            <div class="col-12 col-md-4">
                ...
                <div class="d-flex align-items-center gap-2">
                    ...
                    <input type="text" id="color_preview" class="form-control border-grey"
                        value="{{ old('color', $category->color ?: '#3b82f6') }}" readonly>
                </div>
                @error('color')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            <div class="col-12 col-md-4">
                ...
                <input type="number" name="order" id="order"
                    class="form-control border-grey @error('order') is-invalid @enderror"
            ...
            <div class="col-12 col-md-4">
                ...
                <select name="status" id="status" class="form-select border-grey @error('status') is-invalid @enderror"
                    required>
                    <option value="1" @selected(old('status', $category->status) == 1)>Aktif</option>
                    <option value="0" @selected((string) old('status', $category->status) === '0')>Tidak Aktif</option>
                </select>
                @error('status')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
